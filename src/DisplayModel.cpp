...
RestartLayout:
    ...
    for (int pageNo = 1; pageNo <= PageCount(); ++pageNo) {
        ...
        if ((!gGlobalPrefs->fixedPageUI.hideScrollbars && !gUseOverlayScrollbar) && (!needVScroll) && viewPort.dy < currPosY + rowMaxPageDy) {
            needVScroll = true;
        ...
        //   set to true (i.e., the block has been processed)
        if ((!gGlobalPrefs->fixedPageUI.hideScrollbars && !gUseOverlayScrollbar) && (!needHScroll) &&
    ...
    if ((!gGlobalPrefs->fixedPageUI.hideScrollbars && !gUseOverlayScrollbar) && (!needVScroll) && canvasDy > viewPort.dy) {
        needVScroll = true;
        viewPort.dx -= GetSystemMetrics(SM_CXVSCROLL);
        goto RestartLayout;
    }
    ...
    int canvasDx = windowMargin.left + columnMaxWidth[0] + (columns == 2 ? pageSpacing.dx + columnMaxWidth[1] : 0) +
                   windowMargin.right;
    if ((!gGlobalPrefs->fixedPageUI.hideScrollbars && !gUseOverlayScrollbar) && (!needHScroll) && canvasDx > viewPort.dx) {
        needHScroll = true;
        viewPort.dy -= GetSystemMetrics(SM_CYHSCROLL);
        goto RestartLayout;
    }
